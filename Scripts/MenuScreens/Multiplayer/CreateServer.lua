...
UI.PageServerLoadProfile=
{
	...
	SetPicture = function()
		local iSelection = UI.PageCreateServer.GUI.LevelCombo:GetSelection(0);

		if iSelection then
		
			local szFileName = UI.PageCreateServer.GUI.szLastMap[iSelection];
			
			if (szFileName and strlen(szFileName) > 1) then
			
				--szFileName = "profiles/player/"..getglobal("g_playerprofile").."/savegames/"..szFileName;
				szFileName = "Levels/"..tostring(szFileName).."/loadscreen_"..tostring(szFileName)..".dds";
			
				local iTexture = System:LoadImage(szFileName);
	
				if (iTexture) then
					UI.PageCreateServer.GUI.MapPreview:SetColor("255 255 255 255");
					UI.PageCreateServer.GUI.MapPreview:SetTexture(iTexture);
		...
		UI.PageCreateServer.GUI.MapPreview:SetColor("0 0 0 64");
		UI.PageCreateServer.GUI.MapPreview:SetTexture(nil);
	end,
}
