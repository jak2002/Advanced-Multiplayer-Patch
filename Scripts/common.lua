...
function LookAt(ent1,ent2)
	local p1 = {}
	CopyVector(p1,ent1:GetPos())
	local p2 = {}
	CopyVector(p2,ent2:GetPos())
	local diff = {}
	FastDifferenceVectors(diff,p1,p2)
	local ang = ent1:GetAngles()
	ConvertVectorToCameraAngles(ang,diff)
	ent2:SetAngles(ang)
end

-- Make ent2 look at ent1 with offset
function LookAtFrom(ent1,ent2,pos)
	local p1,p2,diff,ang = {},{},{},{}
	CopyVector(p1,ent1:GetPos())
	CopyVector(p2,ent2:GetPos())
	FastSumVectors(p2,p2,pos)
	FastDifferenceVectors(diff,p1,p2)
	ConvertVectorToCameraAngles(ang,diff)
	ent2:SetAngles(ang)
end
